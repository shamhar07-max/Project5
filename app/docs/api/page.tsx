...
export default function ApiDocs() {
  return <main className="site" style={{ "--a": "#2563eb", "--b": "#7c3aed" } as CSSProperties}>
    <SiteHeader />
    <section className="band-tight page-top">
      <div className="wrap" style={{ maxWidth: 1080 }}>
        ...
        <h3 className="h3 subhead">Authentication</h3>
        <p className="prose-p">Server-to-server: send <code>Authorization: Bearer dbk_…</code> with an organization API key. Keys act only for their organization and only within their scopes: {API_SCOPES.join(", ")}. In the browser, signed-in requests act as the person; add <code>?organization=&lt;id&gt;</code> to act in an organization where they are an active member.</p>
        <h3 className="h3 subhead">Endpoints</h3>
        <div style={{ overflowX: "auto" }}><table className="table"><thead><tr><th>Method</th><th>Path</th><th>Auth</th><th>Scope</th><th>Description</th></tr></thead><tbody>{endpoints.map(e => <tr key={e[0] + e[1]}><td><b>{e[0]}</b></td><td><code>{e[1]}</code></td><td>{e[2]}</td><td>{e[3]}</td><td>{e[4]}</td></tr>)}</tbody></table></div>
        <h3 className="h3 subhead">Pagination, rate limits and request IDs</h3>
        <p className="prose-p">List endpoints accept <code>limit</code> (1–100, default 25) and <code>offset</code>. Every response carries <code>x-request-id</code> (send your own to correlate). Requests are rate limited per key, person or IP; exceeding the limit returns 429.</p>
        <h3 className="h3 subhead">Errors</h3>
