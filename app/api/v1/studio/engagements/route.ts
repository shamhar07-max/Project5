...
export const GET = api(async (req, caller) => {
  requireScope(caller, "read:studio");
  ...
  const rows = await getDb().select().from(engagements).where(and(scope, eq(engagements.service, "studio"))).orderBy(desc(engagements.updatedAt)).limit(limit).offset(offset);
