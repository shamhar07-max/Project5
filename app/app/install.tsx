...
export function InstallPanel() {
  ...
  return <div className="install" aria-live="polite">
    ...
    {state === "installable" && <div className="install-state"><Download size={28} /><div><strong>Ready to install</strong><p>Adds DigitalBurj to your home screen with shortcuts to learning, projects and WhatsApp.</p><button type="button" className="btn btn-primary btn-sm" onClick={() => install()}>Install DigitalBurj</button></div></div>}
    ...
    {(state === "unsupported" || state === "unknown") && <div className="install-state"><Monitor size={28} /><div><strong>Open this page on your phone</strong><p>On Android (Chrome) you will see an install button; on iPhone use Safari’s “Add to Home Screen”. Desktop Chrome and Edge can also install it from the address bar.</p>
      <button type="button" className="btn btn-secondary btn-sm" onClick={async () => { try { await navigator.clipboard.writeText(location.origin + "/app"); setCopied(true); } catch { setCopied(false); } }}><Copy size={16} /> {copied ? "Link copied" : "Copy link"}</button></div></div>}
  </div>;
}
