  ...
  return <main className="public-site" style={{ "--a": "#2563eb", "--b": "#0d9488" } as CSSProperties}>
    ...
    <section className="section" style={{ paddingTop: "8rem" }}>
      <div className="shell">
        ...
        <form className="board-filters"><input name="q" defaultValue={q} placeholder="Search help articles" aria-label="Search help" /><button className="mag mag-red" style={{ minHeight: 48 }}><span className="mag-label">Search</span></button></form>
        {CATEGORIES.map(c => { const items = shown.filter(a => a.category === c); return items.length ? <div key={c} style={{ marginBottom: "1.6rem" }}><h3 className="job-h">{c}</h3><div className="board-list">{items.map(a => <Link key={a.slug} href={`/support/${a.slug}`} className="board-item" data-spotlight><div><strong>{a.title}</strong><span>{a.purpose}</span></div><span /><ArrowUpRight size={20} className="board-arrow" /></Link>)}</div></div> : null; })}
