  ...
  return <main className="site" style={{ "--a": "#2563eb", "--b": "#0d9488" } as CSSProperties}>
    ...
    <section className="band-tight page-top">
      <div className="wrap" style={{ maxWidth: 860 }}>
        <Link href="/support" className="link" style={{ marginTop: 0 }}>← Help center</Link>
        ...
        <h3 className="h3 subhead">Steps</h3><ol className="kb-steps">{a.steps.map((s, i) => <li key={i}>{s}</li>)}</ol>
        <h3 className="h3 subhead">Expected result</h3><p className="prose-p">{a.result}</p>
        <h3 className="h3 subhead">Troubleshooting</h3><ul className="kb-steps">{a.troubleshooting.map((s, i) => <li key={i}>{s}</li>)}</ul>
