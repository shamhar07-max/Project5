...
const allowed = [/^\/brand\/digitalburj-wordmark-(approved|800)\.webp$/, /^\/brand\/db-iconmark\.png$/, /^\/brand\/wordmark\.png$/, /^\/brand\/partners\//, /^\/brand\/hero\/(tower|creative|operations)-(720|1440)\.webp$/];
...
for (const file of files) {
  ...
  if (!file.endsWith("brand-data.ts") && !file.endsWith("scenes.tsx") && !file.endsWith("hero-image.tsx")) {
...
console.log(`Imagery check passed: ${scenes.length} authored scenes, each placed exactly once; hero artwork is registered.`);
